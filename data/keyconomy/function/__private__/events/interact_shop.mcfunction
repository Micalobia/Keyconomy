...
execute as @e[type=interaction,tag=key.shop,distance=..10] at @s on target if entity @s[tag=key.current_player] run tag @n[type=interaction,tag=key.shop] add key.shop.active
...
execute if entity @s[tag=key.owner] run function keyconomy:__private__/events/interact_shop/details
execute if entity @s[tag=!key.owner] run function keyconomy:__private__/events/interact_shop/details
