...
execute if entity @s[tag=!key.shop.creative] run function keyconomy:__private__/shop/hit/customer/give_owner_money with entity @n[type=marker,tag=key.shop.active] data.owner
execute if entity @s[tag=!key.shop.creative] run scoreboard players operation #key.stock key.math -= #key.count key.math
